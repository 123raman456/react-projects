...
function Text() {
  ...
  return (
    <>
      <div className="app">
        {showRes?(<QuizApp score={score} corrAns={corrAns} handleRestart={handleRestart}/>):(<>
        <div className="question-area">
          <h5>Score: {score}</h5>
        
        <div className="question-count">
          <h2>
            QuestionArea {currQuestion + 1}/{questions.length}
          </h2>
          </div>
        ...
        <div className="answer-area">
          {questions[currQuestion].answerOption.map((ans, i) => {
            return (
              <button key={i} onClick={() => handleansOption(ans.isCorrect)}>
                {ans.answerText}
              </button>
            );
          })}
